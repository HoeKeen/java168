...
class CelPhone extends BelPhone {
	...
	public void setPortable(boolean portable) {
		...
		move = (portable) ? "이동중에 " : "이동불가능"; // (오브젝트를)? 트루일때:펄스일때;
	...
	@Override
	public String toString() {
		return String.format("%s인  %s제품을 사용해서  %s 라고 %s 통화한다.", 
				KIND, company, call, this.move); // this는 생략 가능
	}
...
class Iphone extends CelPhone {
	...
	@Override
	public String toString() {
		return String.format("[%s] [%s] 제품을  사용해서 [%s] [%s]을 한다.", 
				company, KIND, move, search); //super가 있으면 바로 위에 클래스에서 호출
	}

	//[애플] 아이폰을 사용해서 이동중에 [뉴스 검색]을   한다.
}
...
class GalPhone extends Iphone{
	...
	@Override
	public String toString() {
		return String.format("[%s] [%s] 제품을 사용해서, [%s] [%s]한다.", 
				company, KIND, move, pay); //super가 있으면 바로 위에 클래스에서 호출
	}
...
public class Phone {
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println("0.종료 1.집전화걸기 2.전화받기 3.휴대폰걸기 "+"\r"+
					 "4.아이폰 서치 5.삼성페이결제");
